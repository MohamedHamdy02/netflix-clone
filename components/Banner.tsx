import Image from "next/legacy/image";
import { useEffect, useState } from "react";
import { baseUrl } from "../constants/movie";
import { Data } from "../typings";
import { FaPlay } from "react-icons/fa";
import { InformationCircleIcon } from "@heroicons/react/outline";
import { useRecoilState } from "recoil";
import { modalState, movieState } from "../atoms/modalAtoms";

interface Props {
  netflixOriginals: Data[];
}

function Banner({ netflixOriginals }: Props) {
  const [movie, setMovie] = useState<Data | null>(null);
  const [currentMovie, setCurrentMovie] = useRecoilState(movieState);
  const [showModal, setShowModal] = useRecoilState(modalState);

  useEffect(() => {
    setMovie(
      netflixOriginals[Math.floor(Math.random() * netflixOriginals.length)]
    );
  }, [netflixOriginals]);

  return (
    <div className="flex flex-col space-y-4 py-16 md:space-y-4 lg:h-[65vh] lg:justify-center lg:pb-12">
      <div className="absolute top-0 left-0 -z-10 h-[95vh] w-screen">
        <Image
          src={`${baseUrl}${movie?.backdrop_path || movie?.poster_path}`}
          layout="fill"
          objectFit="fill"
          property="priority"
          alt="image"
        />
      </div>
      <h1 className="text-2xl font-bold md:text-4xl lg:text-6xl text-[#3bddf9]">
        {movie?.title || movie?.name || movie?.original_name}
      </h1>
      <p className="max-w-xs text-shadow-md text-xs md:max-w-lg md:text-lg lg:max-w-2xl lg:text-xl">
        {movie?.overview.split(" ").splice(0, 30).join(" ")}
      </p>

      <div className="flex space-x-3">
        <button className="bannerButton bg-white text-black">
          <span className="h-4 w-4 text-black md:h-7 md:w-7 text-center contents">
            <FaPlay />
          </span>
          Play
        </button>
        <button
          className="bannerButton bg-[gray]/70"
          onClick={() => {
            setCurrentMovie(movie);
            setShowModal(true);
          }}
        >
          <span className="h-5 w-5 md:h-8 md:w-8">
            <InformationCircleIcon />
          </span>
          More Info
        </button>
      </div>
    </div>
  );
}

export default Banner;
